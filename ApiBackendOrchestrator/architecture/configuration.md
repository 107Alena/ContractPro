...
| Переменная | Описание | По умолчанию | Заметки |
|-----------|----------|-------------|---------|
| `ORCH_DM_BASE_URL` | Base URL REST API DM (используется `dmclient` для бизнес-вызовов) | — (required) | Формат: `http://dm:8080/api/v1`. Суффикс `/api/v1` обязателен — `dmclient` строит URL как `BaseURL + "/documents/..."`, а DM экспонирует роуты под `/api/v1/...` |
| `ORCH_DM_HEALTH_URL` | Base URL DM для health-probe (`/healthz`) | derived from `ORCH_DM_BASE_URL` (отрезается суффикс `/api/v1`) | DM экспонирует `/healthz` в корне, не под `/api/v1`. Если не задан — выводится из `ORCH_DM_BASE_URL`. Задавайте явно только если health-эндпоинт DM висит на другом хосте/порту |
...
# DM Client
# ORCH_DM_HEALTH_URL — по умолчанию выводится из ORCH_DM_BASE_URL отрезанием
# суффикса /api/v1 (DM экспонирует /healthz в корне, не под /api/v1).
# Задавайте явно только если health-эндпоинт DM висит на другом хосте/порту.
# ORCH_DM_HEALTH_URL=http://localhost:8081
