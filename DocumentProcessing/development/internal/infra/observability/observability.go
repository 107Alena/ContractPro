...
// SDK is the composite observability entry point for the Document Processing
...
func New(ctx context.Context, cfg config.ObservabilityConfig) (*SDK, error) {
	...
	tracer, err := NewTracer(ctx, "dp-worker", cfg.TracingEnabled, cfg.TracingEndpoint, cfg.TracingInsecure)
	if err != nil {
		return nil, err
	}
