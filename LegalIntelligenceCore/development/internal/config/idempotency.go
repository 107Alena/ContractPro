...
type IdempotencyConfig struct {
	...
	UserConfirmedProcessingTTL time.Duration // LIC_USER_CONFIRMED_PROCESSING_TTL — lic-user-confirmed SETNX PROCESSING TTL (§6.10 Resume step 2)
...
func loadIdempotencyConfig() IdempotencyConfig {
	return IdempotencyConfig{
		...
		UserConfirmedProcessingTTL: envDuration("LIC_USER_CONFIRMED_PROCESSING_TTL", 90*time.Second),
...
func (i IdempotencyConfig) validate() error {
	...
	if i.UserConfirmedProcessingTTL <= 0 {
		return fmt.Errorf("config: LIC_USER_CONFIRMED_PROCESSING_TTL must be > 0, got %s", i.UserConfirmedProcessingTTL)
	}
	return nil
}
