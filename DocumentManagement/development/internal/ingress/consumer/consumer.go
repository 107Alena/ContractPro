...
func (c *EventConsumer) handleGetSemanticTree(ctx context.Context, body []byte) {
	...
	if strings.TrimSpace(event.VersionID) == "" {
		c.logValidationFailure(topic, event.JobID, fmt.Errorf("missing required field: version_id"))
		return
	}
...
func (c *EventConsumer) handleDiffReady(ctx context.Context, body []byte) {
	...
	if strings.TrimSpace(event.BaseVersionID) == "" || strings.TrimSpace(event.TargetVersionID) == "" {
		c.logValidationFailure(topic, event.JobID,
			fmt.Errorf("missing required fields: base_version_id and/or target_version_id"))
		return
	}
...
// On error: cleans up the key so the next delivery can re-process.
