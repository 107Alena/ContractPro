...
| 7. Документы | 9 + 1 tablet | ContractsListPage | ContractsMetricsStrip, CurrentActions, SearchInput, SortControl, FilterChips, DocumentsTable, PaginationControls, TrustFooter (reuse). FE-TASK-058 (ORCH-TASK-056): колонки Тип/Риск из реальных `contract_type`/`risk_level` (`—` только при null); серверные фильтры (status / risk_level / contract_type[] / processing_status-группы / период) и серверная сортировка (SortControl → API sort/order, client header-сортировка убрана); «высокий риск» в strip — page-scoped из risk_level. Группа «С ошибкой» = PARTIALLY_FAILED/FAILED/REJECTED; AWAITING_USER_INPUT не фильтруется (orchestrator-managed → 400). Badge-контраст рисков — общий a11y-долг (§8.2 (f)), не закрывается этой задачей |
