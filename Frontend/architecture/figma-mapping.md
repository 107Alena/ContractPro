...
**Реализация (упрощение dashboard):** по продуктовому решению экран сокращён до WelcomeBlock (один CTA «Новая проверка договора», размер `lg`, без иконки-лупы) → двухколоночный layout (`Недавние проверки` слева | `Быстрый старт` + `Сводка` + `Организация` справа) → TrustFooter.

**Honesty-отклонения от макета 84:2:** относительно Figma убраны блоки **«Что важно сейчас»** (`CurrentActions`), **«Последняя проверка»** (`LastCheckCard`), **«Статус обработки»** (`ProcessingStatus`) и **«Ключевые риски за последнее время»** (`KeyRisksCards`), а также вторичные CTA WelcomeBlock «Загрузить договор» / «Вставить текст». `CurrentActions` остаётся в коде — переиспользуется на ContractsList; `dashboard-last-check`/`dashboard-processing-status`/`dashboard-key-risks` теперь не используются страницей (orphan-виджеты, оставлены в кодовой базе).
