...
// allowedInternal is the EXACT set of first-party packages the orch Status
// Publisher's non-test source may import. Three entries — the domain model +
// ports (the cross-domain wire contracts) and the broker package (sentinel
// errors + the Publish signature seam contract; recorded as R2 — documented
// exception to the otherwise infra-free egress allowlist, same rationale as
// the sibling dm publisher). PublishOutcome is a LOCAL MIRROR (seams.go) of
// metrics.PublishOutcome, pinned in seams_test.go against the metrics SSOT
// (the universal base.Outcome / router.CallOutcome / cost.Outcome /
// schemavalidator.RepairOutcome / dm.PublishOutcome local-mirror precedent —
// keeps the production source hermetic; the metrics import lives in the
// _test file). There is NO internal/config import (local Config,
// 036/047-injected), NO internal/infra/observability/metrics import in
// production (concrete prometheus is forbidden — Metrics is seamed away), NO
// internal/application/* (the publisher is a stateless port implementation),
// NO internal/egress/publisher/dm import (sibling-not-parent — both
// publishers share the SHAPE of their seam stack but each owns its own copy
// so future Orchestrator-facing publishers can grow in this package without
// re-importing dm), NO third-party path. Reviewer gate: size == 3 EXACTLY.
...
// forbiddenInternal is the active-fail set: packages whose presence in
// allowedInternal (a future "consistency" edit) would silently break the
// hermeticity contract. TestHermeticImports asserts they are NOT in the
// allowlist BEFORE scanning, so a regression fails loudly here rather than
// passing the import scan because someone widened the allowlist. Notably
// internal/infra/observability/metrics (the parent of labels): would pull a
// concrete prometheus dependency in. Notably internal/application/pipeline:
// the publisher is a leaf port impl, the orchestrator imports IT, not vice
// versa. Notably internal/egress/publisher/dm: the sibling-not-parent
// rationale above.
var forbiddenInternal = []string{
	...
	"contractpro/legal-intelligence-core/internal/egress/publisher/dm",
...
// would reverse the dep edge — or internal/egress/publisher/dm —
// sibling-not-parent) ever lands in the allowlist, and flags any
...
func TestHermeticImports(t *testing.T) {
	if len(allowedInternal) != 3 {
		t.Fatalf("allowedInternal MUST have exactly 3 entries (reviewer gate); got %d", len(allowedInternal))
	}
...
// file in the package (the dmawaiter / pipeline / aggregator / dm
