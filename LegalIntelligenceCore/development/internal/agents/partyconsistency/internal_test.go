...
// code-architect CC-2): stdlib + internal/domain/{model,port} + the sibling
// agent packages it composes (base, promptbuilder — used STRUCTURALLY here for
// b.ValidationFacts, the mandated §3 difference) + the embedded-asset loaders
// (prompts, schemas) + the shared DP-faithful EXTRACTED_TEXT decoder
// (artifacts). DELIBERATELY excludes internal/config (resolved values are
...
// DocumentProcessing module (the local minimal documentStructure mirror in
// spec.go owns the DP-faithful party_details shape — the Agent-1 local
// projection precedent; artifacts centralises ONLY EXTRACTED_TEXT).
...
// TestHermeticImports). Without this test the no-cross-module-import rule that
// JUSTIFIES the local documentStructure decode (code-architect Q2) is
// unenforced.
...
// file in the package (same approach as base/keyparams TestGofmtClean).
