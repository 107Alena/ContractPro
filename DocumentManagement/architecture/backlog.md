...
| ~~12~~ | ~~**Count-by-artifact_status агрегат**~~ | ~~`GET /documents/stats` + repo `CountCurrentVersionsByArtifactStatus` + индекс `(organization_id, artifact_status)`. Источник для дашборд-метрики «в работе» (потребитель — Orchestrator `GET /contracts/stats`). Детали: [../tasks.json](../tasks.json) DM-TASK-059~~ | ~~Высокий~~ | Готово (DM-TASK-059): [api-specification.yaml](api-specification.yaml) `GET /documents/stats`, [high-architecture.md](high-architecture.md) §6.9.1, [storage.md](storage.md) индексы `idx_documents_org_status` + `idx_versions_org_artifact_status`, [integration-contracts.md](integration-contracts.md) §3.1 |
