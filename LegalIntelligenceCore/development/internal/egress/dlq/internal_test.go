...
// allowedInternal is the EXACT set of first-party packages the DLQ
// publisher's non-test source may import. Three entries — the domain model
// + ports (the cross-domain wire contracts and the DLQPublisherPort) and
// the broker package (sentinel errors traversed by the broker-outcome
// classifier; same R2 exception as the sibling dm / orch publishers).
...
// internal/config import (local Config, 047-injected), NO
// internal/infra/observability/metrics import in production (concrete
// prometheus is forbidden — Metrics is seamed away), NO
// internal/infra/observability/logger import (HashContent is byte-capped
// at 1024 and locked to log-field use; HashPayload here is the FULL-bytes
// twin — architect Q1), NO internal/application/* (the publisher is a
// stateless port implementation), NO third-party path.
...
// allowlist BEFORE scanning, so a regression fails loudly here rather
...
// if any forbidden internal lands in the allowlist, and flags any
// third-party import (prometheus / amqp091 / otel — all behind seams or
// out of scope here).
func TestHermeticImports(t *testing.T) {
	...
	for _, forbidden := range forbiddenInternal {
		if _, present := allowedInternal[forbidden]; present {
			t.Fatalf("allowedInternal must NOT contain %q — the DLQ publisher is hermetic (stdlib + model/port/broker(sentinels) only); telemetry/clock/logger are seamed, broker.Publish is behind a seam, Config is a ctor param", forbidden)
		}
	}
