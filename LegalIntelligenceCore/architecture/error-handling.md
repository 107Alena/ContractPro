...
|--------------|-------|-----------|------------------|
| `INVALID_MESSAGE_SCHEMA` | `STAGE_RECEIVE` | false | (только в DLQ; не публикуется в Orch — события из DM считаются доверенными) |
| `INVALID_ORG_ID_MISMATCH` | `STAGE_RECEIVE` | false | (DLQ; alert) |

### 3.2 DM-related errors

| `error_code` | Stage | Retryable | UserMessage |
|--------------|-------|-----------|-------------|
| `DM_ARTIFACTS_TIMEOUT` | `STAGE_REQUESTING_ARTIFACTS` | true | «Не удалось получить данные документа за отведённое время. Попробуйте ещё раз.» |
| `DM_ARTIFACTS_MISSING` | `STAGE_ARTIFACTS_RECEIVED` | true | «Данные документа не найдены. Возможно, обработка ещё не завершилась.» |
| `DM_PERSIST_FAILED` | `STAGE_PUBLISHING_ARTIFACTS` | false (если DM is_retryable=false) / true (иначе) | «Не удалось сохранить результат анализа.» (если non-retryable: «Документ был удалён или недоступен.») |
| `DM_PERSIST_TIMEOUT` | `STAGE_AWAITING_DM_CONFIRMATION` | true | «Не удалось получить подтверждение сохранения. Попробуйте позже.» |

### 3.3 Agent-related errors

| `error_code` | Stage | Retryable | UserMessage |
|--------------|-------|-----------|-------------|
| `AGENT_OUTPUT_INVALID` | `STAGE_AGENT_*` | true | «Не удалось получить корректный анализ. Запустите повторную проверку.» |
| `AGENT_TIMEOUT` | `STAGE_AGENT_*` | true | «Один из этапов анализа занял слишком много времени.» |
| `AGENT_INPUT_TOO_LARGE` | `STAGE_AGENT_*` | false | «Документ слишком большой для анализа. Разделите его на части.» |
| `AGENT_DEPENDENCY_FAILED` | `STAGE_AGENT_*` | (зависит от cause) | «Не удалось завершить анализ из-за сбоя на предыдущем этапе.» |

### 3.4 LLM-provider errors

| `error_code` | Stage | Retryable | UserMessage |
|--------------|-------|-----------|-------------|
| `LLM_ALL_PROVIDERS_FAILED` | `STAGE_AGENT_*` | true | «Сервис анализа временно недоступен. Попробуйте позже.» |
| `LLM_QUOTA_EXCEEDED` | `STAGE_AGENT_*` | false (escalate) | «Превышен лимит запросов к ИИ-сервису. Обратитесь к администратору.» |
| `LLM_CONTENT_POLICY_VIOLATION` | `STAGE_AGENT_*` | false | «Документ содержит контент, который не может быть обработан.» |
...
| `INPUT_REJECTED` | `STAGE_RECEIVE` / `STAGE_ARTIFACTS_RECEIVED` | false | «Полученные данные документа повреждены или неполные.» |
| `DOCUMENT_TOO_LARGE` | `STAGE_ARTIFACTS_RECEIVED` | false | «Документ слишком большой для юридического анализа.» |

### 3.6 User-confirmation errors

| `error_code` | Stage | Retryable | UserMessage |
|--------------|-------|-----------|-------------|
| `USER_CONFIRMATION_EXPIRED` | `STAGE_AWAITING_USER_CONFIRMATION` | false | «Время на подтверждение типа договора истекло. Запустите проверку заново.» |
| `INVALID_CONTRACT_TYPE` | `STAGE_AWAITING_USER_CONFIRMATION` | false | «Указан некорректный тип договора. Обновите страницу и попробуйте снова.» |
...
| `IDEMPOTENCY_STORE_UNAVAILABLE` | `STAGE_RECEIVE` | true | (NACK, не публикуется в Orch немедленно) |
...
- `lic_agent_repair_outcome_total{agent, provider, outcome}` (`outcome` ∈ `repaired_ok | repair_failed | repair_provider_error`). `provider` = `used_provider` (sticky per OQ-10 / `llm-provider-abstraction.md` §2.1). Авторитетный enum фиксируется в `observability.md` §3.3.
