...
function buildActions(results: AnalysisResults): ActionItem[] {
  ...
  if (recommendations.length > 0) {
    actions.push({
      ...
      // Без конкретного числа: на ResultPage рекомендации показываются внутри
      // карточек рисков (по risk_id), а полный массив может содержать
      // несопоставленные/дублирующие записи — счётчик расходился бы с видимым.
      description: 'Готовые формулировки — в карточках рисков: раскройте «Показать формулировку».',
    });
  }
