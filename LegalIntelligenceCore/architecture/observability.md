...
```
INFO  pipeline.start          {correlation_id, job_id, version_id, organization_id, mode:"INITIAL|RE_CHECK"}
...
Allowed in logs:
- IDs: `correlation_id`, `job_id`, `document_id`, `version_id`, `organization_id`, `requested_by_user_id`, `confirmed_by_user_id`, `agent_id`, `provider_id`, `message_id`.
...
lic_agent_repair_outcome_total{agent,outcome}                  counter (outcome ∈ repaired_ok|repair_failed)
```
...
`outcome` ∈ `success | repair_success | invalid_output | provider_error | timeout`.
...
lic_llm_input_tokens_total{provider,model,agent}               counter
...
```
lic_prompt_injection_detected_total{agent,severity}             counter
...
- `correlation_id`, `job_id`, `version_id`, `document_id`, `organization_id`, `requested_by_user_id`.
- На root: `lic.pipeline.mode` (INITIAL/RE_CHECK), `lic.pipeline.outcome`.
- На agent spans: `lic.agent.id`, `lic.agent.outcome`, `lic.agent.repair_attempts`.
- На llm spans: `lic.llm.provider`, `lic.llm.model`, `lic.llm.input_tokens`, `lic.llm.output_tokens`, `lic.llm.latency_ms`, `lic.llm.cost_usd`, `lic.llm.fallback_used: bool`.
