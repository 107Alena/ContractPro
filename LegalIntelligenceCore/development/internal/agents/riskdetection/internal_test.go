...
// package's non-test source may import — BYTE-IDENTICAL to mandatoryconditions'
// allowlist (code-architect CC-4). Hermeticity invariant (CLAUDE.md /
// code-architect): stdlib + internal/domain/{model,port} + the sibling agent
// packages it composes (base, promptbuilder — for Content only; Agent 5, like
// Agent 4, mints no structural block) + the embedded-asset loaders (prompts,
// schemas) + the shared DP-faithful artifact decoder (artifacts — the
// LIC-TASK-026 steward decision). DELIBERATELY excludes internal/config
...
// base), and the DocumentProcessing module: BOTH SEMANTIC_TREE and the
// optional PROCESSING_WARNINGS are byte-faithful passthroughs (the deliberate
// ABSENCE of a local processingWarnings mirror struct is the
// SEMANTIC_TREE-passthrough class, not the Agent-3 DOCUMENT_STRUCTURE
// local-decode class — CC-4), and the artifacts package owns the local
// minimal DP-faithful EXTRACTED_TEXT struct.
...
// partyconsistency / mandatoryconditions TestHermeticImports).
