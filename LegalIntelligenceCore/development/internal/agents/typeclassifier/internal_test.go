...
// loaders (prompts, schemas). DELIBERATELY excludes internal/config (resolved
// values are constructor params, config→value mapping is LIC-TASK-047's job —
// the router.RouterConfig hermeticity precedent), internal/infra/* and
// internal/llm/* (telemetry/router are seamed inside base), and the
// DocumentProcessing module (artifacts are decoded via local minimal structs).
...
// allowlist — no internal/config, no concrete infra/llm, no third-party
// (mirrors base.TestHermeticImports / schemavalidator.TestSingleThirdPartyImport).
