...
// code-architect Q5): stdlib + internal/domain/{model,port} + the sibling
// agent packages it composes (base, promptbuilder) + the embedded-asset
// loaders (prompts, schemas) + the shared DP-faithful artifact decoder
// (artifacts — the LIC-TASK-026 steward decision). DELIBERATELY excludes
// internal/config (resolved values are constructor params, the config→value
// mapping is LIC-TASK-047's job — the router.RouterConfig hermeticity
// precedent), internal/infra/* and internal/llm/* (telemetry/router are
// seamed inside base), and the DocumentProcessing module (the artifacts
// package owns the local minimal DP-faithful structs).
...
// DocumentProcessing module (mirrors base.TestHermeticImports /
// typeclassifier.TestHermeticImports).
