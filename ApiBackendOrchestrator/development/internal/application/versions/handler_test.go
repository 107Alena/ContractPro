...
// uuidV4Regexp matches the canonical UUID v4 textual form. Used by tests that
// capture-and-reuse a job_id generated by jobid.NewJobID() (which is invariant
// — not swappable via the h.uuidGen seam).
...
func TestHandleUpload_Success(t *testing.T) {
	...
	// jobID is generated via jobid.NewJobID() (ORCH-TASK-052/054) — non-deterministic,
	// so capture from the response and reuse for downstream equality checks.
	if !uuidV4Regexp.MatchString(resp.JobID) {
		t.Fatalf("job_id = %s, want UUID v4", resp.JobID)
	}
	jobID := resp.JobID
	...
	// Verify X-Correlation-Id header (still seam-driven: uuid-001 is the
	// first h.uuidGen() call in HandleUpload).
	...
	if dm.createCalls[0].Req.JobID != jobID {
		t.Errorf("DM CreateVersion JobID = %s, want %s", dm.createCalls[0].Req.JobID, jobID)
	}
	...
	if pub.calls[0].JobID != jobID {
		t.Errorf("published JobID = %s, want %s", pub.calls[0].JobID, jobID)
	}

	// Verify Redis tracking key uses the same job_id.
	if len(kv.calls) != 1 {
		t.Fatalf("expected 1 Redis Set call, got %d", len(kv.calls))
	}
	expectedKVKey := "upload:org-001:" + jobID
	if kv.calls[0].Key != expectedKVKey {
		t.Errorf("redis key = %s, want %s", kv.calls[0].Key, expectedKVKey)
	}
}
...
func TestHandleUpload_PublishedCommandFields(t *testing.T) {
	...
	cmd := pub.calls[0]
	// job_id is generated via jobid.NewJobID() — capture-and-reuse against
	// the response (the upload handler embeds the same value).
	var resp VersionUploadResponse
	if err := json.NewDecoder(rr.Body).Decode(&resp); err != nil {
		t.Fatalf("decode response: %v", err)
	}
	if !uuidV4Regexp.MatchString(cmd.JobID) {
		t.Errorf("cmd.JobID = %s, want UUID v4", cmd.JobID)
	}
	if cmd.JobID != resp.JobID {
		t.Errorf("cmd.JobID = %s, want %s", cmd.JobID, resp.JobID)
	}
...
func TestHandleUpload_S3KeyFormat(t *testing.T) {
	...
	// Key format: uploads/{org_id}/{job_id}/{file_uuid}.
	// job_id is non-deterministic (jobid.NewJobID); file_uuid is seam-driven
	// and equals uuid-002 since h.uuidGen is called exactly twice per request
	// (correlation_id then file_uuid). See anchor comment in handler.go.
	if len(dm.createCalls) != 1 {
		t.Fatalf("expected 1 CreateVersion call to capture jobID")
	}
	jobID := dm.createCalls[0].Req.JobID
	if !uuidV4Regexp.MatchString(jobID) {
		t.Fatalf("captured jobID = %s, want UUID v4", jobID)
	}
	expectedPrefix := "uploads/org-001/" + jobID + "/uuid-002"
	if storage.putCalls[0].Key != expectedPrefix {
		t.Errorf("S3 key = %s, want %s", storage.putCalls[0].Key, expectedPrefix)
	}
}

// =========================================================================
// ORCH-TASK-054: job_id wire-up — HandleUpload (RE_UPLOAD) job_id checks
// =========================================================================

// TestHandleUpload_JobIDPopulatedOnDMRequest asserts that the DM
// CreateVersionRequest carries a non-empty UUID v4 JobID even though the
// `job_id,omitempty` JSON tag would silently omit a missing value.
func TestHandleUpload_JobIDPopulatedOnDMRequest(t *testing.T) {
	dm := &mockDMClient{}
	h := newTestHandlerFull(dm, &mockStorage{}, &mockPublisher{}, &mockKVStore{})
	...
	if dm.createCalls[0].Req.JobID == "" {
		t.Fatal("DM CreateVersionRequest.JobID must be set (cross-domain correlation key)")
	}
	if !uuidV4Regexp.MatchString(dm.createCalls[0].Req.JobID) {
		t.Errorf("DM CreateVersionRequest.JobID = %s, want UUID v4", dm.createCalls[0].Req.JobID)
	}
}

// TestHandleUpload_JobIDFlowsThroughDMAndPublisher asserts that the same
// job_id is propagated end-to-end: response.JobID == DM CreateVersionRequest.JobID
// == ProcessDocumentCommand.JobID == Redis tracking key suffix == embedded
// in the S3 path namespace.
func TestHandleUpload_JobIDFlowsThroughDMAndPublisher(t *testing.T) {
	dm := &mockDMClient{}
	storage := &mockStorage{}
	pub := &mockPublisher{}
	kv := &mockKVStore{}
	...
	jobID := resp.JobID
	if !uuidV4Regexp.MatchString(jobID) {
		t.Fatalf("response.JobID = %s, want UUID v4", jobID)
	}
	...
	if dm.createCalls[0].Req.JobID != jobID {
		t.Errorf("DM CreateVersionRequest.JobID = %s, want %s", dm.createCalls[0].Req.JobID, jobID)
	}
	...
	if pub.calls[0].JobID != jobID {
		t.Errorf("ProcessDocumentCommand.JobID = %s, want %s", pub.calls[0].JobID, jobID)
	}

	if len(kv.calls) != 1 {
		t.Fatalf("expected 1 Redis Set call, got %d", len(kv.calls))
	}
	wantKVKey := "upload:org-001:" + jobID
	if kv.calls[0].Key != wantKVKey {
		t.Errorf("Redis key = %s, want %s", kv.calls[0].Key, wantKVKey)
	}
	...
	if !strings.Contains(storage.putCalls[0].Key, "/"+jobID+"/") {
		t.Errorf("S3 key = %s, must embed job_id namespace /%s/", storage.putCalls[0].Key, jobID)
	}
}

// =========================================================================
// ORCH-TASK-054: job_id wire-up — HandleRecheck (RE_CHECK) job_id checks
// =========================================================================

// TestHandleRecheck_JobIDPopulatedOnDMRequest asserts that the DM
// CreateVersionRequest carries a non-empty UUID v4 JobID for the recheck flow.
func TestHandleRecheck_JobIDPopulatedOnDMRequest(t *testing.T) {
	dm := &mockDMClient{
		getVerFn: func(_ context.Context, _, _ string) (*dmclient.DocumentVersionWithArtifacts, error) {
			return stubVersionWithArtifacts("FULLY_READY"), nil
		},
	...
	if dm.createCalls[0].Req.OriginType != "RE_CHECK" {
		t.Errorf("OriginType = %s, want RE_CHECK", dm.createCalls[0].Req.OriginType)
	}
}

// TestHandleRecheck_JobIDFlowsThroughDMAndPublisher asserts the recheck flow
// propagates the same job_id end-to-end: response.JobID == DM
// CreateVersionRequest.JobID == ProcessDocumentCommand.JobID == Redis tracking
// key suffix. Recheck does not touch S3 (it reuses parent's source file).
func TestHandleRecheck_JobIDFlowsThroughDMAndPublisher(t *testing.T) {
	dm := &mockDMClient{
		getVerFn: func(_ context.Context, _, _ string) (*dmclient.DocumentVersionWithArtifacts, error) {
			return stubVersionWithArtifacts("FULLY_READY"), nil
		},
		createVerFn: func(_ context.Context, _ string, _ dmclient.CreateVersionRequest) (*dmclient.DocumentVersion, error) {
			return &dmclient.DocumentVersion{VersionID: "ver-new-001", VersionNumber: 2}, nil
		},
	}
	pub := &mockPublisher{}
	kv := &mockKVStore{}
