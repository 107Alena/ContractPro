...
// code-architect D10): stdlib + internal/domain/{model,port} + the sibling
// agent packages it composes (base, promptbuilder — for Content only; Agent 8
// mints no structural block) + the embedded-asset loaders (prompts, schemas).
//
// 6-entry, artifacts-FREE — the Agent-6 non-EXTRACTED_TEXT-consumer CLASS, a
// STRICT subset of the Agent-4/5/7 allowlist with internal/agents/artifacts
// DELIBERATELY DROPPED. The §8 envelope (detailed_report.txt:33-43) has no
// <contract_document> block and §8 "Зависимости"
// (ai-agents-pipeline.md:1280) lists only SEMANTIC_TREE from DM — Agent 8
// consumes NO EXTRACTED_TEXT, so the shared DP-faithful artifacts.ExtractedText
// decoder is dead weight here. The artifacts-drop is the deliberate
// "non-EXTRACTED_TEXT consumer" class, NOT an omission: re-adding artifacts to
// "match" Agents 4/5/7 is a regression a reviewer must NOT make (the
// riskdetection "deliberate absence is a class, not an omission" house style).
// Also DELIBERATELY excludes internal/config (resolved values are constructor
// params, the config→value mapping is LIC-TASK-047's job — the
...
// internal/agents/artifacts (the deliberate Agent-6-class divergence; mirrors
// base / typeclassifier / keyparams / partyconsistency / mandatoryconditions /
// riskdetection / recommendation / summary TestHermeticImports).
func TestHermeticImports(t *testing.T) {
	...
	if _, present := allowedInternal[artifactsPkg]; present {
		t.Fatalf("allowedInternal must NOT contain %q — Agent 8 consumes no EXTRACTED_TEXT (§8 envelope has no <contract_document>); the artifacts-drop is the deliberate Agent-6-class divergence (D10)", artifactsPkg)
	}
